...
"""
modified from https://github.com/stanford-crfm/helm/blob/main/scripts/fact_completion/create_benchmark.py
...
def main():
    ...
    for i in range(len(seed_relations_df)):
        ...
        paraphrases = seed_relations_df.iloc[i]["paraphrase"]
        paraphrases = paraphrases[2:-2].split("', '")
        all_paraphrases[relation] = paraphrases
    ...
    for head_qid, prop in selected:
        ...
        if not prop in templates.keys():
            print(f"Missing template for {prop}")
            print(templates)
            print(relation)
            continue
        ...
        paraphrases = []
        for paraphrase in all_paraphrases[prop]:
            paraphrase = paraphrase.replace("[X]", query_name)
